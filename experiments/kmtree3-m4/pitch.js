...
(function (root) {
  ...
  var NAMED_NEAREST_GAP = 1.05;
  var LINE_CLEAR = R + 0.3;

  function clamp(v, a, b) { return v < a ? a : v > b ? b : v; }
  function dotInField(p) { return { x: clamp(p.x, LINE_CLEAR, W - LINE_CLEAR), y: clamp(p.y, LINE_CLEAR, L - LINE_CLEAR) }; }
  ...
  /* m1 (the follow-up in reviews/codex-verify-r4.md, item 2): the named-
   * nearest push ends with a re-push, and nothing spaced the dots after it,
   * so a pushed man could land on another (1 picture in 15,558 at 1.27 m).
   * After the final re-push, each pushed man who is still closer than `min`
   * to the ball's man, or within NAMED_CLEAR of anybody, is moved to the
   * nearest free spot on or just outside the `min` circle round the man on
   * the ball (straight out along his own line first, then turning either
   * way). Display only. */
  var NAMED_CLEAR = 1.6;
  function settleNamed(pos, moved, hp, min, T) {
    function freeAt(id, p, gap) {
      return Object.keys(pos).every(function (o) { return o === id || dist(pos[o], p) >= gap; });
    }
    Object.keys(moved).forEach(function (id) {
      var p0 = pos[id];
      if (!p0) return;
      if (dist(p0, hp) >= min - 1e-6 && freeAt(id, p0, NAMED_CLEAR)) return;
      var a0 = Math.atan2(p0.y - hp.y, p0.x - hp.x), best = null;
      /* never nearer the goal than he was (a man left behind the ball stays behind it) */
      var depth0 = T ? (p0.y - hp.y) * dirOf(T) : Infinity;
      [MIN_GAP, NAMED_CLEAR].some(function (gap) {
        /* straight out along his own line from the ball first (so a man left
         * behind the ball stays behind it), then turning a little either way */
        for (var k = 0; k <= 18 && !best; k++) {
          for (var r = min; r <= min + 4 && !best; r += 0.5) {
            [1, -1].some(function (sg) {
              var a = a0 + sg * k * Math.PI / 18, c = dotInField({ x: hp.x + Math.cos(a) * r, y: hp.y + Math.sin(a) * r });
              if (dist(c, hp) >= min - 1e-6 && (!T || (c.y - hp.y) * dirOf(T) <= Math.max(depth0, 0) + 1e-6) && freeAt(id, c, gap)) { best = c; return true; }
              return false;
            });
          }
        }
  ...
  function nearOf(st, p, holder) {
    ...
    var m = /([^ .,:]+) is the nearest of (your|their) players/.exec(String(mo.text || ''));
    if (m) {
      var named = byFirst(st, m[1], m[2] === 'your' ? 'you' : 'them');
      if (named && named !== holder && onPitch(st, named) && teamOf(st, named) !== ht && !isKeeper(named, st.squad) && !isKeeper(named, st.opp)) return named;
    }
  ...
  function startOf(st, p) {
    ...
    var S = {
      ...
      near: nearOf(st, p, holder), nearText: /nearest of (?:your|their) players/.test(String(mo.text || '')),
      crossTo: crossTo, special: special, air: air, via: via
    };
  ...
  function freeze(st, S, pos, opts) {
    ...
    if (sc === 'tired_gap') tidy(pos, fixed, FREEZE_GAP);
    if (S.near && S.nearText && sc !== 'tired_gap' && pos[S.near.id]) {
      var nd = dist(pos[S.near.id], hp), moved = {};
      function pushNamedNearest(q) {
        if (q === S.near || (opts.beaten && q === opts.beaten) || !pos[q.id]) return;
        var p0 = pos[q.id], d = dist(p0, hp), min = nd + NAMED_NEAREST_GAP;
        if (d >= min) return;
        var ux = (p0.x - hp.x) / (d || 1), uy = (p0.y - hp.y) / (d || 1);
        if (!d) { ux = q.slot % 2 ? 1 : -1; uy = 0; }
        pos[q.id] = dotInField({ x: hp.x + ux * min, y: hp.y + uy * min });
        var p1 = pos[q.id], dx = p1.x - hp.x, dy = p1.y - hp.y, d1 = Math.sqrt(dx * dx + dy * dy);
        if (d1 < min) {
          var sy = dy < 0 ? -1 : 1;
          var need = Math.sqrt(Math.max(0, min * min - dx * dx));
          pos[q.id] = dotInField({ x: p1.x, y: hp.y + sy * need });
        }
        moved[q.id] = 1;
      }
      squadOf(st, O).players.forEach(pushNamedNearest);
      if (Object.keys(moved).length) {
        tidy(pos, fixed, FREEZE_GAP);
        squadOf(st, O).players.forEach(pushNamedNearest);
        settleNamed(pos, moved, hp, nd + NAMED_NEAREST_GAP, T);   /* m1: spacing after the final re-push */
      }
    }
    return pos;
  }
  ...
  function after(st, S, sc, R, pos, fixed, hp, ball, T, O, dir) {
    ...
    if (S.near && S.nearText && sc !== 'tired_gap' && pos[S.near.id]) {
      /* When the text names the nearest opponent, keep the rest of that side
       * at least a metre farther away after shape and spacing have had their say. */
      var nd = dist(pos[S.near.id], hp), moved = {};
      squadOf(st, O).players.forEach(function (q) {
        if (q === S.near || !pos[q.id]) return;
        var p0 = pos[q.id], d = dist(p0, hp), min = nd + NAMED_NEAREST_GAP;
        if (d >= min) return;
        var ux = (p0.x - hp.x) / (d || 1), uy = (p0.y - hp.y) / (d || 1);
        if (!d) { ux = q.slot % 2 ? 1 : -1; uy = 0; }
      ...
      if (Object.keys(moved).length) {
        ...
        squadOf(st, O).players.forEach(function (q) {
          if (q === S.near || !pos[q.id]) return;
          var p0 = pos[q.id], d = dist(p0, hp), min = nd + NAMED_NEAREST_GAP;
          if (d < min) pos[q.id] = dotInField({ x: hp.x + (p0.x - hp.x) / (d || 1) * min, y: hp.y + (p0.y - hp.y) / (d || 1) * min });
        });
        settleNamed(pos, moved, hp, nd + NAMED_NEAREST_GAP, T);   /* m1: spacing after the final re-push */
      }
    }
