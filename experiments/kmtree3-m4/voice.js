...
(function (root) {
  ...
  function momentLine(p, ctx) {
    ...
    var pick = null;
    /* m1: "Your team still has the ball." (the kept-ball opener) names no man; the sentence that does is later */
    for (var i = 0; i < ss.length && !pick; i++) if (/ has the ball\b| is going to | has a (free kick|corner)| is through\b/.test(ss[i]) && !/^Your team still has the ball/.test(ss[i])) pick = ss[i];
    if (!pick) pick = ss[0];
    pick = clean(pick);
