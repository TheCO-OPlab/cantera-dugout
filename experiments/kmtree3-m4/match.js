...
(function (root) {
  ...
  var X1_DEFAULT = X1_ASK ? x1Parse(X1_ASK) : { long: true };
  ...
  function chainCapOf(st) { return CHAIN_CAP + (st.x1 && st.x1.cap ? 1 : 0); }
  function winCapOf(st) { return WIN_CAP + (st.x1 && st.x1.cap ? 1 : 0); }
  ...
  function choose(st, index) {
    ...
    if (o.mineVal !== null && o.themVal !== null) {
      ...
      if (rule && rule.dice > 1) {
        rolls = [d1];
        for (var dk = 1; dk < rule.dice; dk++) { var dx = 1 + Math.floor(st.rng.next() * 6); rolls.push(dx); if (dx > d1) d1 = dx; }
      }
     ...
     * gets it back) */
    if (fx && o.subOff && o.actor && fx.realSubs('you') && fx.substitute('you', o.subOff, o.actor, p.minute)) {
    ...
    if (st.chainMode && typeof p.tzone === 'number') {
      ...
      if (picked.effect !== 'concede' && picked.win && (p.flipOk || o.counterWin)) {
        ...
        var starter = fx && picked.fxTo ? picked.fxTo : o.counterWin && o.to ? o.to : wz >= 1 && o.actor && o.actor.line === 0 ? M.carrierFor(wz, st.squad, legs(st)) : o.actor;
    ...
    if (GUARD.lastWin && st.chainMode && st.chain && st.chain.next === 'zone' && p.moment.sit.who === 'them' && st.n + 1 >= momentsOf(st)) {
    ...
    if (st.chainMode && !st.chain && !st.handoff && p.moment.sit.who === 'you' && (ev.kind === 'nothing' || ev.kind === 'kept') &&
      ['keep', 'back', 'hold', 'probe'].indexOf(o.pays) >= 0 && /keeps? (it|the ball)|starts again|has the ball/.test(keptTxt)) {
  ...
  function fxAfter(st, p, o, ev, picked, band, tierFrom, dice) {
    ...
    if (dice) E(band === 'good' ? 'clean_win' : band === 'mixed' ? 'half_win' : 'loss');
    ...
    if (o.shotBy) E('shot', { actor: o.shotBy });
