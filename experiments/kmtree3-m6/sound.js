  ...
  P.playCue = function (name, t) {
    ...
    switch (name) {
      ...
      case 'soundoff':  // ui: about to mute, a softer falling pair
        ...
        break;
      case 'buildcard':   // m5 (ruling q3): a card your build made arrives in its own extra place: a soft rising shimmer, not in CUES
      case 'buildcard2':  // the same with a fourth note, for two build cards on one menu
        this.mallet(s, t, 784.0, 0.07, 0.3);
        this.mallet(s, t + 0.07, 987.8, 0.065, 0.3);
        this.mallet(s, t + 0.14, 1318.5, 0.06, name === 'buildcard2' ? 0.3 : 0.45);
        if (name === 'buildcard2') this.mallet(s, t + 0.21, 1568.0, 0.055, 0.45);
        this.noiseBurst(s, { t: t + 0.03, f: 7200, Q: 1.4, dur: 0.32, gain: 0.012, attack: 0.06 });
        break;
      case 'select':
