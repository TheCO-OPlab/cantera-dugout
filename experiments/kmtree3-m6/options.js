...
(function (root) {
  ...
  var GUARD = { air: true, stack: true, dest: true, say: true, passer: true, nearest: true, form: true, aerial: true, merge: true, sureGoal: true, offRunner: true, noTrap: true, keeperFresh: true, realSafe: true, blockFloor: true, overlapScene: true, keepTwo: true, ghostFree: true, placedHonest: true, fxSlot: true, riskStar: true, keeperOut: true, headerHonest: true, offsideStop: true, namesRolled: true, namesReceiver: true, wingScene: true, leftBehind: true };
  /* m6: KM_RISKSTAR=off (node) is m5's rule: a risky starred card greyed
   * (the checks that compare a match with s0 run score.js this way) */
  if (typeof process !== 'undefined' && process.env && process.env.KM_RISKSTAR === 'off') GUARD.riskStar = false;
  /* (the page: ?riskstar=off, used by buildviewcheck E1 to compare with m2) */
  try { if (/[?&]riskstar=off\b/.test((root.location && root.location.search) || '')) GUARD.riskStar = false; } catch (e) { }
    ...
    },
    {
      ...
      build: function (x) {
        ...
        return {
          ...
          pays: hold ? 'claimhold' : 'claim', table: hold ? R.CONSEQUENCE.them.claimhold : R.CONSEQUENCE.them.claim, names: { tgt: first(x.foil) }, tgtMan: x.foil,   /* m6: the man who scores if it goes wrong (headline.js scorerOf) */
     ...
     * ball is on the ground, so nobody heads it */
    {
      ...
      build: function (x) {
        ...
        return {
          ...
          pays: x.state && x.state.bounced ? 'slidestop' : 'slide', names: { tgt: first(x.foil) }, tgtMan: x.foil,   /* m6: the man who scores if it goes wrong (headline.js scorerOf) */
    ...
    },
    {
      ...
      build: function (x) {
        ...
        return {
          ...
          pays: x.flipOk ? 'divehold' : 'dive', names: { tgt: first(x.foil) }, tgtMan: x.foil,   /* m6: the man who scores if it goes wrong (headline.js scorerOf) */
  ...
  /* m6: o's good result carries an edge into the next duel (a CARRY grant)
   * and q's good result carries none */
  function starEdgeOver(o, q) {
    var ge = function (z) { return ((z.grants && z.grants.good) || []).some(function (c) { return CARRY[c.id]; }); };
    return !!(o.unlock && ge(o) && !ge(q));
  }
  function offer1(ctx, howMany) {
    ...
    (fx && FXL.GUARD.pool ? POOL.concat(fx.poolEntries()) : POOL).forEach(function (o) {
      ...
      built.push({
        ...
        /* m6: who scores when a keeper's claim or dive misses (headline.js); a give and go built after a shot (prove-ruling) */
        tgtMan: b.tgtMan || null, ggAfterShot: !!b.ggAfterShot
      });
    });
    ...
    if (GUARD.noTrap && (zoneMode || ctx.zoneEscape || tMode || ctx.boxStep)) {
      ...
      built.filter(function (o) { return o.unlock && !o.disabled; }).forEach(function (o) {
        var left = built.filter(function (q) { return !q.disabled; });
        if (GUARD.riskStar && ub(o) >= 0.5) o.riskStar = Math.round(ub(o) * 36);   /* m6: the warning, on every risky starred card */
        if (left.length <= 2) return;
        var b0 = ub(o), why = null;
        var short = null;
        /* m6 (designer ruling 2026-09-26, "risky starred cards stay
         * playable"): a starred card that goes wrong half the time or more is
         * no longer greyed. Like a power card, the worst play on this turn can
         * be right for the bigger plan: the card warns (o.riskStar, the page's
         * risk line) and does not forbid. KMOptions.setGuard('riskStar', false)
         * is m5's rule. It is still judged against the others on every count
         * below (that is not about risk). */
        if (b0 >= 0.5 && GUARD.riskStar) {
          /* (o.riskStar is set above; this is a card m5 greyed) */
          o.riskUngreyed = true;
        } else if (b0 >= 0.5) {
        ...
        if (why) { /* m5's risk grey */ }
          ...
          /* m6: "at least as well on every count" skips one count: the edge a
           * good result carries into the next duel ("then +2 to you next").
           * A starred card whose good result carries one that the other
           * card's does not is better on that count, so it is not greyed as
           * beaten by it (7% of the starred cards greyed this way in m5). */
          var edgeOver = beat.filter(function (q) { return starEdgeOver(o, q); });
          if (edgeOver.length) o.edgeKept = true;
          /* (with the rule off, m5's grey of such a card is marked: prove-ruling reads it) */
          if (!GUARD.riskStar && edgeOver.length && beat.length) o.edgeGreyedM5 = true;
          if (GUARD.riskStar) beat = beat.filter(function (q) { return !starEdgeOver(o, q); });
          beat = beat[0];
    ...
    /* m6 (ruling item 2): A RISKY STARRED CARD THAT m5 GREYED KEEPS A PLACE.
     * m5 showed it greyed; cutting it now would make it vanish. When the cut
     * leaves it off, it takes the place of the weakest ordinary card (not
     * starred, not a build card, not the low-risk one), and that card is
     * greyed, so the menu still holds three. Not when it waits its turn on
     * your attack (every starred card does, KW_SLOT) or plays exactly like a
     * card already shown. */
    if (GUARD.riskStar) live.forEach(function (o) {
      if (!o.riskUngreyed || o.disabled || o.hide || out.indexOf(o) >= 0 || held(o)) return;
      if (out.some(function (q) { return effectTV(q, o) < 0.01; })) return;
      var safeNow = out.filter(function (q) { return q.pinned === 'safe'; })[0] || (out.indexOf(safe) >= 0 ? safe : null);
      var weak = out.filter(function (q) { return !q.unlock && !q.fxCreated && q !== safeNow; })
        .sort(function (a, b) { return worth(a) - worth(b); })[0];
      if (!weak) return;
      weak.disabled = true; weak.dominated = true; weak.cutGrey = true; weak.pinned = null; weak.riskBump = true;
      weak.greyWhy = 'Greyed: the menu holds three cards and this one does least here.';
      weak.check += ' The menu holds three cards and this one does least here, so it is shown greyed out.';
      out[out.indexOf(weak)] = o;
    });
    ...
    if (GUARD.ghostFree) {
      ...
      built.forEach(function (o) {
        ...
        var alt = out.filter(function (q) { return domBy(q, o) && !(GUARD.riskStar && starEdgeOver(o, q)); })[0];
        if (alt) {
    ...
    /* m6: A RISKY STARRED CARD NEVER MAKES A FOURTH. The three places hold
     * at most three plain and starred cards. If a risky starred card that
     * m5 would have greyed ever stands as a fourth, the weakest ordinary
     * card (not starred, not a build card, not the low-risk one) is greyed
     * in its place, never the starred card. (The cut above keeps three, so
     * this is a guard: slotcheck S2 counts every menu.) */
    if (GUARD.riskStar && out.filter(function (o) { return !o.fxCreated; }).length > want && out.some(function (o) { return o.riskStar; })) {
      var safe1 = out.filter(function (o) { return o.pinned === 'safe'; })[0] || (out.indexOf(safe) >= 0 ? safe : null);
      var weak = out.filter(function (o) { return !o.unlock && !o.fxCreated && o !== safe1; })
        .sort(function (a, b) { return worth(a) - worth(b); })[0];
      if (weak) {
        weak.disabled = true; weak.dominated = true; weak.cutGrey = true; weak.pinned = null; weak.riskBump = true;
        weak.greyWhy = weak.greyWhy || 'Greyed: the menu holds three cards and this one does least here.';
        weak.check += ' The menu holds three cards and this one does least here, so it is shown greyed out.';
        out.splice(out.indexOf(weak), 1);
      }
    }
  ...
  var API = { sizes: function () { return { SHOT_CAP: SHOT_CAP, THEIR_CAP: THEIR_CAP, CLOSE_HARD: CLOSE_HARD, CLOSE_PLACED: CLOSE_PLACED, THEIR_CLOSE: THEIR_CLOSE, RUN_COST: RUN_COST, TOUCH_COST: TOUCH_COST }; }, fkTaker: fkTaker, setUnlockSizes: setUnlockSizes, MENU_LIVE: MENU_LIVE, MENU_GREY: MENU_GREY, worth: worth, setPinStrong: function (v) { PIN_STRONG = v; }, setNarrow: function (v) { NARROW_BY = v; }, setPinDef: function (v) { PIN_DEF = v; }, setBoxSave: function (v) { BOX_SAVE_BONUS = v; }, setYourSetPieces: function (fk, lg) { YOUR_FK = fk; YOUR_LONG = lg; }, setTEdge: function (v) { T_EDGE = v; }, setSafeDef: function (v) { SAFE_DEF = v; }, setSafeAtt: function (v) { SAFE_ATT = v; }, setClose: function (h, p) { CLOSE_HARD = h; CLOSE_PLACED = p; }, setShotCap: function (v) { SHOT_CAP = v; }, setPlaceCap: function (v) { PLACE_CAP = v; }, setRealSafe: function (v) { REALSAFE_AT = v; }, setTheirClose: function (v) { THEIR_CLOSE = v; }, setPassCap: function (v) { PASS_CAP = v; }, setTheirCap: function (v) { THEIR_CAP = v; }, setSetPieces: setSetPieces, POOL: POOL, offer: offer, verdict: verdict, shortOf: shortOf, iconOf: iconOf, annotate: annotate, overlapBack: overlapBack, overlapScene: overlapScene, theirWingScene: theirWingScene, SIDE_WORD: SIDE_WORD, ICONS: ICONS, END_ICON: END_ICON, tag: tag, statOf: statOf, setKwSlot: setKwSlot, setGuard: function (k, v) { GUARD[k] = v; }, SHOT_PAYS: SHOT_PAYS, GUARD: GUARD,
    effectTV: effectTV, starEdgeOver: starEdgeOver, markerOf: markerOf, lane: lane, T_AT: T_AT, T_PRIMARY: T_PRIMARY, CARRY: CARRY, carryText: carryText, ZONE_NAME: ZONE_NAME, ZONE_AT: ZONE_AT, ZONE_PRIMARY: ZONE_PRIMARY, CONTINUES: CONTINUES };
  root.KMOptions = API;
