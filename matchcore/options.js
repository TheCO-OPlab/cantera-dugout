...
(function (root) {
  ...
  var CARRY = {
    ...
    unmarked: { amount: 2,
      ...
      applies: function (c, o) { return ['Z_SHOOT', 'Z_PLACE', 'Z_CHIP'].indexOf(o.id) >= 0 && o.actor === c.man; } },
    beaten: { amount: 2,
      text: function (c) { return first(c.man) + ' is out of position: +2 to your next check against him'; },
    ...
    set: { amount: 2,
      text: function (c) { return 'Your defence was back in place before ' + first(c.man) + ' got there: +2 to your next check against ' + first(c.man); },
    ...
    twoBeaten: { amount: 3,
      text: function (c) { return first(c.man) + ' got past ' + first(c.a) + ' and ' + first(c.b) + ', so they are a player short: +3 to your next check'; },
      why: function (c) { return first(c.man) + ' got past two of their players'; },
      applies: function () { return true; } },
    running: { amount: 2,
      text: function (c) { return first(c.man) + ' has the ball running at their goal: +2 to his next check'; },
    ...
    },
    {
      ...
      build: function (x) {
        return {
           ...
           * nothing at all */
          restLine: x.act.legs,
          label: 'Substitute ' + first(x._sub) + ' on, with fresh legs',
          read: 'Your ' + ({ def: 'defence', mid: 'midfield', att: 'attack' }[x.act.legs]) +
            ' is down to ' + Math.round(x.legs) + ' out of 100 and losing Pace and Physical because of it. ' +
      ...
      build: function (x) {
        ...
        return {
          ...
          read: tag(h, 'technique', x.legs) + ' against ' + tag(d1, 'defending', 100) + ', with ' + first(d2) +
            ' next to ' + first(d1) + '. Past both and ' + first(h) + ' is at the edge of their box with +3 to the next check. ' +
    ...
    },
    {
      ...
      build: function (x) {
        ...
        return {
          ...
          does: { good: 'leaves his line, gets to the pass first and throws it to ' + first(m), mixed: 'leaves his line, gets to the pass first and kicks it clear',
  ...
  function offer1(ctx, howMany) {
    ...
    POOL.forEach(function (o) {
      ...
      if ((zoneMode || ctx.boxStep || tMode) && ctx.carried && mineV !== null) {
        var probeO = { id: o.id, actor: t.mine, foil: t.theirs };
      ...
      if (!GUARD.stack) ups.forEach(function (u) { if (u !== top) { mods.push({ n: u.n, why: u.why }); uses.push({ name: u.name, n: u.n, text: u.text || null, why: u.why }); } });
      ...
      if (zoneMode && ctx.state && typeof ctx.state.counterplay === 'function' && mineV !== null && themV !== null) {
        cplay = ctx.state.counterplay({ id: o.id, actor: t.mine, foil: t.theirs, to: b.to || null, pays: b.pays });
      ...
      var ch = R.odds(edge === null ? 0 : edge);
      ...
      if (GUARD.merge) {
        var sameKey = function (x) {
          return [x.effect, x.move, x.tmove, !!x.win, x.via, x.into, x.mode, x.to && x.to.id, !!x.rebound, !!x.caught,
            x.trip && x.trip.id, x.theirTo && x.theirTo.id, x.edgeNote || '',
      ...
      built.push({
        ...
        cost: cost, restLine: b.restLine || null,
    ...
    if (zoneMode) built.forEach(function (o) {
      ...
      o.dropBy = null; o.sameAs = null; live.push(o);
